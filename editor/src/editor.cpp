...
static Texture gravRangeTex;
static bool showGravRanges;
...
static void showLevelObject(ImDrawList *drawList, const std::shared_ptr<ObjectModel>& object)
{
    ...
    if (planet && showGravRanges)
    {
        // All gravity ranges seem to have this hard-coded scale at the moment...
        constexpr float GRAV_RANGE_SCALE = 3.f;

        float scaledGravWidth = gravRangeTex.width * object->scale / 5 * GRAV_RANGE_SCALE;
        float scaledGravHeight = gravRangeTex.height * object->scale * GRAV_RANGE_SCALE;
        ...
        drawList->AddImage(gravRangeTex.id, screenRangeStart, screenRangeEnd, uv0, uv1);
    }
}
...
void initEditor()
{
    ...
    IM_ASSERT(loadTextureFromFile("../assets/textures/range.png", gravRangeTex));
    showGravRanges = true;
}
